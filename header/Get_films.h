...
#define GET_FILMS
    ...
    struct strFilm
    {
        ...
        friend std::ostream& operator<< (std::ostream& stream, const strFilm& film) {
            // stream << "id: " << film.id << ", name: " << film.name << ", localName: " << film.localName <<  ", rating: " << film.rating  << std::endl;
            stream << "Название фильма: " << film.localName <<  ", Рейтинг: " << film.rating  << std::endl;
            return stream;
        }
    };
    ...
    struct strInputData
    {
        ...
        friend std::ostream& operator<< (std::ostream& stream, const strInputData& inputData) {
            stream << "year: " << inputData.year <<  ", path_to_year: " << inputData.pathToYear << "path_to_rating: " << inputData.pathToRating <<  ", path_to_name: " << inputData.pathToName  << std::endl;
            return stream;
        }
    ...
    bool getPath(int argc,
                char *argv[],
                struct strInputData &input_data);

    void tokenize(std::string const &str,
                    const char delim,
                    std::vector<std::string> &out);
    
    int modStoi(std::string str,
                int* pValue);
